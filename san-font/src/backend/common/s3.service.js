...
class S3Service {
  ...
  async uploadFileToS3({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        params.ContentDisposition = `inline; filename=${encodeURIComponent(
          realName
        )}`;
  ...
  async removeArrayFileS3({ bucket = S3_BUCKET, arrayFile }) {
    return Promise.all(
      arrayFile.map((fileName) => this.removeFileS3({ bucket, fileName }))
    );
  }
  ...
  async getSignedUrl({
    ...
    return new Promise((resolve) => {
      ...
      const location = `${folder}/${fileName.replace(
        /[^a-zA-Z]/g,
        ""
      )}_${uuidv4()}.${type}`;
      const params = {
