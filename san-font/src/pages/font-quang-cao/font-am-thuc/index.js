				...
				{NameFont.map((item, id) => {
					return (
						<div className='bg-white shadow-md' key={id}>
							...
							<Link href={`/font-am-thuc/${item.id}`}>
...
export async function getStaticProps() {
	...
	const datas = data.filter(e => e.nameFont === 'Font ẩm thực')
...
// 	const response = await fetch(`http://localhost:4000/HomeData`)
// 	const data = await response.json()
// 	const NameFont = data.filter(ev => ev.nameFont === 'Font ẩm thực')

// 	return {
// 		props: { NameFont, pageName: id },
// 	}
// }
